...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    // Prefer the signed session cookie for normal requests.
    ...
    // A valid session may arrive before its user row has been created locally.
    if (!user) {
      if (ENV.oAuthServerUrl) {
        try {
          const userInfo = await this.getUserInfoWithJwt(sessionToken ?? "");
          ...
          // OAuth sync failed, fall through to local session creation
        }
      }
